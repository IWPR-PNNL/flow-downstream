...
2. Calculate an alternative unit flow value based on an intensity factors
3. Calculate a source inflow connecting Node C to Node B
4. Calculate a discharge outflow connecting Node B to Node C
...
### Calculation type (data position 2)

The value of the calculation type must be one of the following:
A_collect
B_calculate
C_source
D_discharge
...
### Primary Node Information (data position 3-8)
...
### Secondary Node Information (data position 9-14)
These data inputs follow the same structure as the primary node but are used to describe the node that is being linked to the primary node as either a source (inflow from) or a target node (discharge to).

### Parameter (data position 15)

This data item identifies the type of value in the value column (position 16).

### Value (data position 16)

This data item provides one of the following: (1) an input flow value, (2) an intensity coefficient (3) a source flow fraction, or (4) a discharge flow fraction

## Creating input data for different calculations

### Collect values

To build flows between nodes and calculate cross-unit flows, initial flow values are necessary. For example, if the amount of energy required to withdraw a gallon of public water from a larger water supply is a desired flow calculation, then the amount of water withdrawn from the water supply by the water supply sector is a required input. Initial values must be supplied in the following way:

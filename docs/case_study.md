...
===============
Nearly all sectors included have energy in-flows (demand), energy out-flows (discharge), water in-flows (withdrawals), and water out-flows (discharge). The methodology for determining each of these is described below within each sector section.
...
Water withdrawal - Average water intensity values are provided for both surface and coal mines from [SOURCE], shown below:
...
Discharge - Very few coal mines exist in coastal areas of the united states. Therefore, following in line with assumptions made in [Llnl SOURCE], 100% of water not consumed in the production of coal is
...
Water consumption -  water consumption estimates for corn growth for ethanol used in biomass are expected to follow the same consumption estimates for all crop irrigation. This information is provided directly in in Dieter et al. [SOURCE].

The average consumption values for ethanol growth for biomass in the US are:
 - Fresh groundwater: 77%
 - Fresh surface water: 77%

 Water discharge - water discharge estimates for corn growth for ethanol used in biomass are expected to follow the same discharge estimates for all crop irrigation. All water discharged from crop irrigation is expected to be to the surface.
...
The majority of water use estimates are from the 2015 USGS report documenting water use in the US, which provide water values at the county-level for a number of variables
...
Your Pages site will use the layout and styles from the Jekyll theme you have selected in your [repository settings](https://github.com/kmongird/flow-downstream/settings/pages). The name of this theme is saved in the Jekyll `_config.yml` configuration file.
